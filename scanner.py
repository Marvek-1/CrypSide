    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            INSERT INTO signals (
                ...
                score, regime, reason_trace, logic_version, config_version, policy_version
            )
            VALUES (
                %(signal_id)s, %(pair)s, %(ts)s, %(side)s, %(entry)s, %(stop_loss)s, %(take_profit)s,
                %(score)s, %(regime)s, %(reason_trace)s::jsonb, %(logic_version)s, %(config_version)s, %(policy_version)s
            )
            ON CONFLICT (signal_id) DO NOTHING
            """,
            {**sig, "reason_trace": json.dumps(sig["reason_trace"]), "policy_version": POLICY_VERSION},
        )
        conn.commit()
